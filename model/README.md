...
<name>: <value>, …}))` over the record's own stable-identity fields — the same
recipe the STIX projection mints §2.9 ids with
([`piiat_mitrecar/ids.py`](../piiat_mitrecar/ids.py)). *Which* fields identify
each artefact's row is a rule, declared as data in
[`piiat_mitrecar/spindle.yml`](../piiat_mitrecar/spindle.yml) (a map only names
its entry); this directory is that registry's materialized snapshot:

- **`identity.yml`** — every registry entry resolved against the live maps:
  `name`, `map`, `variants`, `car_object`, `car_action`, `scope`, the ordered
  `identity` (`name ← source path on the event`), the positional `fallback`
  (`SourceImage`, `RecordId`) — plus the literal namespaces (`STIX_NS`,
  `CAR_NS`, `SPINDLE_NS`), the mint rule and the scope vocabularies.
- **`record.yml`** — the shape of a spindle: what a minted-identity CAR row
  *is* (the common header, `native.spindle_key`, `native.spindle_scope`, the
  linkage back to its artefact) and its invariants, in the `car/objects`
  convention.

Both are generated by `python model/generate.py` (or
...
--check` (CI) and `tests/test_spindle_model.py`: registry ↔ maps ↔ engine, and
committed snapshot ↔ fresh rendering. See `docs/CAR-Pipeline.md` §7.1.
